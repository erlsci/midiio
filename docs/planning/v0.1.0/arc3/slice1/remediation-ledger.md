...
| 2 | **Deadlock regression test:** close-during-active-delivery does **not** hang | eunit: virtual input + high-rate `send` loop + concurrent `close`, looped, under a timeout that pre-fix code trips | S1 | ✅ done | eunit `close_during_active_delivery_test_` (20 rounds, `flood` sender + concurrent `close(In)`, `{timeout,60}`). **Proven on real ALSA:** `make vm-test` (Ubuntu 24.04 multipass VM, real `/dev/snd/seq`, where `mm_in_stop` *does* `pthread_join` the recv thread — the platform that actually deadlocks) → **All 33 tests passed + ASAN-OK**. Pre-fix this row hangs on ALSA; post-fix it passes. macOS/CoreMIDI also passes (no join there). | |
